...
**L2-də nə sınır:** ARP keş zəhərlənməsi, MAC daşqını, dupleks uyğunsuzluğu, broadcast tufanları, səhv konfiqurasiya edilmiş VLAN trunkları, köçürmədən sonra hələ də səhv VLAN-da qalan port. Ətraflı: [Ethernet və ARP](./ethernet-arp.md).
...
**L4-də nə sınır:** firewallun portu blokladığı, NAT qutusunda dolu əlaqə cədvəli, TCP pəncərə dayanmaları, SYN daşqınları, səhv interfeysə bağlanmış xidmətlər. Ətraflı: [TCP və UDP](./tcp-udp.md).
...
- Qardaş dərslər: [TCP/IP Modeli](./tcp-ip-model.md) · [Ethernet və ARP](./ethernet-arp.md) · [TCP və UDP](./tcp-udp.md) · [Portlar və Protokollar](./ports-protocols.md)
